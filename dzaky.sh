...
check_license() {
    if [ ! -f "$LICENSE_FILE" ]; then
        echo -e "${RED}Lisensi file tidak ditemukan. Meminta lisensi baru...${RESET}"
        request_license
    fi
    ...
    if [ "$LICENSE_KEY" == "DZ4KY" ]; then
        ...
        echo -e "${RED}Lisensi mu ga valid ajg.${RESET}"
        return 1
    fi
}

# Memeriksa lisensi sebelum menjalankan script lebih lanjut
if check_license; then
    echo -e "${GREEN}Lisensi valid. Lanjut menjalankan script...${RESET}"
    # Tempatkan perintah script lainnya di sini
else
    echo -e "${RED}Lisensi tidak valid. Keluar dari script.${RESET}"
    exit 1
fi
